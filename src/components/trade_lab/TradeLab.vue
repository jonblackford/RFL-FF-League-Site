...
const getLocalPackageValue = ({
  ...
  players.reduce((total, player) => total + getTradePlayerValue(player), 0) +
  picks.reduce((total, pick) => total + getDraftPickTradeValue(pick), 0) +
  Math.max(0, faab) * 0.1;
  ...
  (_values, _oldValues, onCleanup) => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const teamAValue = getLocalPackageValue({
        ...
        faab: teamAFaab.value,
      });
      const teamBValue = getLocalPackageValue({
        players: teamBOutgoingPlayers.value,
        picks: teamBPicks.value,
        faab: teamBFaab.value,
      });
      tradeQuote.value = estimateLocalTradeQuote({ teamAValue, teamBValue });
    }, 250);
