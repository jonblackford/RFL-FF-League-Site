...
import { type TradeValueRequestPayload } from "@/api/tradeValuesApi";
import {
...
const getRostersWithDraftPicks = () =>
  props.rosters.map((roster) => ({
    ...roster,
    draftPicks:
      props.request?.rosters.find((requestRoster) => requestRoster.id === roster.id)
        ?.draftPicks ?? [],
  }));
...
watch(
  ...
  async ([rosterId, request, demoSuggestionsByRoster]) => {
    ...
    if (!request && props.rosters.length === 0) {
      suggestions.value = [];
      finderLoading.value = false;
      finderError.value = "";
      return;
    ...
    finderError.value = "";
    try {
      const localSuggestions = generateLocalTradeSuggestions({
        forRosterId: rosterId,
        assetFilter: assetFilter.value,
        rosters: getRostersWithDraftPicks(),
        starterPlayerIdsByRoster: props.starterPlayerIdsByRoster,
      });
      if (currentRequestId === finderRequestId) {
        suggestions.value = localSuggestions.filter(matchesAssetFilter);
      }
    } catch (error) {
      if (currentRequestId !== finderRequestId) return;
      console.error("Unable to generate local trade suggestions:", error);
      suggestions.value = [];
