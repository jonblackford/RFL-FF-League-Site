...
describe("Sleeper API data transforms", () => {
  ...
  test("loads requested players from Sleeper public data without a backend", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockFetchResponse(200, {
        "4046": {
          player_id: "4046",
    ...
    const playersMap = await getPlayersByIdsMap(["4046", "4984"]);

    expect(fetchMock).toHaveBeenCalledOnce();
    expect(fetchMock.mock.calls[0][0]).toBe(
      "https://api.sleeper.app/v1/players/nfl"
    );
    expect(playersMap.get("4046")).toEqual({
      player_id: "4046",
      name: "Patrick Mahomes",
      position: "QB",
      team: "KC",
    });
    expect(playersMap.get("4984")).toEqual({
      player_id: "4984",
      name: "Josh Allen",
      position: "QB",
      team: "BUF",
  ...
  test("searches the backend player endpoint with a bounded result set", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain(
      "https://backend.example/api/getPlayer?query=chase&limit=8"
    );
  ...
  test("searches Sleeper public player data without a backend", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockFetchResponse(200, {
        "4046": {
          player_id: "4046",
    ...
    const players = await searchPlayers("mahomes", 8);
  ...
  test("returns player ids by requested name and team in order", async () => {
    vi.stubEnv("VITE_PLAYER_ID_LOOKUP", "https://lookup.example/player-id");
    const fetchMock = vi.fn().mockResolvedValue(
      mockFetchResponse(200, {
        players: [
